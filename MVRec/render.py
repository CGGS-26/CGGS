...
from gaussian_renderer import render_d
import torchvision
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        rendering = render_d(view, gaussians, pipeline, background)["render"]
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, video: bool):
    with torch.no_grad():
        ...
        scene = Scene(dataset, gaussians, None, False, 3, 3, load_iteration=iteration, shuffle=False)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, video=args.video)